...
def _tag_weight(tag: str, profile: SignalPropertyProfile) -> float:
    mapping = {
        ...
        "markov": 0.65 * profile.closure_need_score + 0.35 * profile.multiscale_score,
        "koopman": 0.4 * profile.multiscale_score + 0.35 * profile.trend_score + 0.25 * (1.0 - profile.unpredictability_score),
        "spectral": 0.4 * profile.oscillatory_score + 0.4 * profile.multiscale_score + 0.2 * (1.0 - profile.unpredictability_score),
        "adiabatic": 0.6 * profile.multiscale_score + 0.4 * profile.trend_score,
